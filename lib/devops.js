...
import { sendDevOpsNotification, buildSiteDownEmbed, buildSiteUpEmbed, buildSSLWarningEmbed, buildRepeatedAlertEmbed } from './devops-slack'
...
async function handleStatusChange(site, previousStatus, newStatus, healthCheck) {
  ...
  if ((previousStatus === 'healthy' || previousStatus === 'degraded') && newStatus === 'down') {
    ...
    // Send first repeated alert (Alert 1 of 5)
    if (!incident.duplicate) {
      try {
        const embed = buildRepeatedAlertEmbed(site, healthCheck, 1, 5, incident.started_at || new Date().toISOString())
        await sendDevOpsNotification(embed)

        // Update incident with alert tracking
        await supabase
          .from('devops_incidents')
          .update({ alerts_sent: 1, last_alert_at: new Date().toISOString() })
          .eq('id', incident.id)
      } catch (error) {
...
/**
 * Process the alert queue — send follow-up alerts for open downtime incidents
 * Called every minute by the devops-alerts cron
 */
export async function processAlertQueue() {
  // Find open downtime incidents that still need alerts
  const { data: incidents, error } = await supabase
    .from('devops_incidents')
    .select(`
      id, site_id, alerts_sent, max_alerts, last_alert_at, started_at,
      devops_sites (id, name, url, environment)
    `)
    .eq('incident_type', 'downtime')
    .neq('status', 'resolved')
    .gt('alerts_sent', 0)
    .order('last_alert_at', { ascending: true })

  if (error) {
    console.error('Failed to query alert queue:', error)
    return { processed: 0, error: error.message }
  }

  // Filter to incidents that still need alerts
  const pending = incidents?.filter(i => i.alerts_sent < (i.max_alerts || 5)) || []
  ...
  for (const incident of pending) {
    // Enforce 55-second minimum gap between alerts
    if (incident.last_alert_at) {
      const elapsed = Date.now() - new Date(incident.last_alert_at).getTime()
    ...
    // Quick health check to verify site is still down before alerting
    try {
      let healthUrl
      if (site.health_url) {
        healthUrl = site.health_url
      ...
      const response = await fetch(healthUrl, {
        method: 'GET',
        headers: { 'User-Agent': 'VA-DevOps-Monitor/1.0' },
        signal: controller.signal,
      })
      clearTimeout(timeoutId)

      // Site recovered — skip alert, autoResolve will handle it on next health check
      if (response.ok) {
        console.log(`Site ${site.name} appears recovered, skipping alert`)
        continue
      }
    } catch (checkError) {
      // Site still down (timeout or error) — proceed with alert
    }

    const nextAlertNumber = incident.alerts_sent + 1
    const maxAlerts = incident.max_alerts || 5

    try {
      const embed = buildRepeatedAlertEmbed(site, null, nextAlertNumber, maxAlerts, incident.started_at)
      await sendDevOpsNotification(embed)

      await supabase
        .from('devops_incidents')
        .update({
          alerts_sent: nextAlertNumber,
          last_alert_at: new Date().toISOString(),
        })
        .eq('id', incident.id)

      sent++
      console.log(`Sent alert ${nextAlertNumber}/${maxAlerts} for ${site.name}`)
    } catch (alertError) {
      console.error(`Failed to send alert for ${site.name}:`, alertError)
    }
  }

  return { processed: sent, pending: pending.length }
}
