...
import { getAnalyticsSummary, getCachedAnalytics, getDetailedAnalytics, getSiteById } from './search-console';
import { generateAIRecommendations, generateSEOPlan, formatPlanForClaudeCode } from './gemini-seo';
import { createProject } from './projects';
import { createPlan } from './seo-plans';
import { listKeywords, buildKeywordPerformance, toQueryMap } from './seo-keywords';
...
export async function generateReport(siteId) {
  ...
  let previousDetailed = null;
  try {
    currentDetailed = await getDetailedAnalytics(siteId, currentStartDate, currentEndDate);
    previousDetailed = await getDetailedAnalytics(siteId, previousStartDate, previousEndDate);
    if (currentDetailed && previousDetailed) {
      currentDetailed.previousQueries = previousDetailed.queries;
    }
  } catch (detailErr) {
    console.error('Error fetching detailed analytics:', detailErr);
  }

  // Build tracked-keyword performance from the detailed query data we already
  // fetched. If detailed data isn't available, leave empty rather than failing
  // the report.
  let trackedKeywords = [];
  try {
    const keywords = await listKeywords(siteId);
    if (keywords.length > 0 && currentDetailed?.queries && previousDetailed?.queries) {
      trackedKeywords = buildKeywordPerformance(
        keywords,
        toQueryMap(currentDetailed.queries),
        toQueryMap(previousDetailed.queries)
      );
    } else if (keywords.length > 0) {
      trackedKeywords = keywords.map((kw) => ({ ...kw, current: null, previous: null, positionDelta: null }));
    }
  } catch (err) {
    console.error('Failed to compute tracked keyword performance:', err);
  }
  ...
  try {
    seoPlan = await generateSEOPlan(
      ...
      monthComparisonData.previousMonth,
      changes,
      { detailedAnalytics: currentDetailed, trackedKeywords }
    );
  ...
  // Persist the plan to seo_plans/seo_plan_tasks tables
  let persistedPlan = null;
  try {
    persistedPlan = await createPlan(siteId, seoPlan);
    console.log('Persisted SEO plan:', persistedPlan?.id);
  } catch (persistError) {
    console.error('Failed to persist SEO plan:', persistError);
  }
 ...
 */
async function storeReport({ siteId, startDate, endDate, metrics, changes, recommendations, aiSummary, seoPlan, projectId, monthComparison, trackedKeywords }) {
  ...
  // Return with extended fields at top level for easy access
  return {
    ...data,
    recommendations: reportData.recommendations,
    month_comparison: reportData.month_comparison,
    tracked_keywords: reportData.tracked_keywords
  };
}

/**
 * Get all reports across all sites (joined with site info)
 */
export async function getAllReports(limit = 20) {
  const supabase = getSupabase();
  ...
  return (data || []).map(report => {
    ...
    if (report.recommendations?.month_comparison) {
      result.month_comparison = report.recommendations.month_comparison;
      result.ai_summary = report.recommendations.ai_summary;
      result.seo_plan = report.recommendations.seo_plan;
      result.project_id = report.recommendations.project_id;
      result.tracked_keywords = report.recommendations.tracked_keywords || [];
      result.recommendations = report.recommendations.recommendations || [];
    }
    return result;
  });
}
  ...
  return (data || []).map(report => {
    if (report.recommendations?.month_comparison) {
      return {
        ...
        tracked_keywords: report.recommendations.tracked_keywords || [],
...
export async function getReportById(reportId) {
  ...
  if (data && data.recommendations?.month_comparison) {
    ...
    data.tracked_keywords = data.recommendations.tracked_keywords || [];
