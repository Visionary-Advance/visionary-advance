 ...
 * @param {string} siteUrl - The site URL from Search Console
 * @param {string} googleEmail - Google account email for API access
 * @param {string|null} displayName - Display name for the site
 * @param {string|null} leadId - CRM lead ID to link this site to
 */
export async function addSite(siteUrl, googleEmail, displayName = null, leadId = null) {
  ...
  // Only include lead_id if provided (to avoid overwriting existing links)
  if (leadId) {
    siteData.lead_id = leadId;
  }
 ...
 * @param {object} updates - Fields to update (display_name, lead_id, business_id, is_active)
 */
export async function updateSite(siteId, updates) {
  const supabase = getSupabase();

  const allowedFields = ['display_name', 'lead_id', 'business_id', 'is_active', 'plan_tier'];
  const updateData = {
    updated_at: new Date().toISOString()
  };
...
/**
 * Link an SEO site to a CRM lead
 * @param {string} siteId - The site ID
 * @param {string} leadId - The CRM lead ID
 */
export async function linkSiteToLead(siteId, leadId) {
  return updateSite(siteId, { lead_id: leadId });
}

/**
 * Link an SEO site to a Business
 * @param {string} siteId - The site ID
 * @param {string} businessId - The Business ID
 */
export async function linkSiteToBusiness(siteId, businessId) {
  return updateSite(siteId, { business_id: businessId });
}
 ...
 * Returns query-level and page-level detail with position, CTR, device, and country breakdowns
 */
export async function getDetailedAnalytics(siteId, startDate, endDate) {
  const supabase = getSupabase();
  ...
  // Build detailed query map with position + CTR
  const queryMap = new Map();
  cached.forEach(day => {
    (day.queries || []).forEach(q => {
      const key = q.keys?.[0] || q.query;
      if (!key) return;
      const existing = queryMap.get(key) || { clicks: 0, impressions: 0, positionSum: 0, ctrSum: 0, days: 0 };
      existing.clicks += q.clicks || 0;
      existing.impressions += q.impressions || 0;
      existing.positionSum += q.position || 0;
      existing.ctrSum += q.ctr || 0;
      existing.days += 1;
      queryMap.set(key, existing);
    });
  });

  const queries = Array.from(queryMap.entries()).map(([query, d]) => ({
    query,
    clicks: d.clicks,
    impressions: d.impressions,
    avgPosition: d.days > 0 ? d.positionSum / d.days : 0,
    avgCtr: d.days > 0 ? d.ctrSum / d.days : 0
  }));

  // Build detailed page map with position + CTR
  const pageMap = new Map();
  cached.forEach(day => {
    (day.pages || []).forEach(p => {
      const key = p.keys?.[0] || p.page;
      if (!key) return;
      const existing = pageMap.get(key) || { clicks: 0, impressions: 0, positionSum: 0, ctrSum: 0, days: 0 };
      existing.clicks += p.clicks || 0;
      existing.impressions += p.impressions || 0;
      existing.positionSum += p.position || 0;
      existing.ctrSum += p.ctr || 0;
      existing.days += 1;
  ...
  cached.forEach(day => {
    (day.devices || []).forEach(d => {
      const key = d.keys?.[0] || d.device;
      if (!key) return;
      const existing = deviceMap.get(key) || { clicks: 0, impressions: 0 };
  ...
  const devices = Array.from(deviceMap.entries()).map(([device, d]) => ({
    device,
    clicks: d.clicks,
    impressions: d.impressions
  }));
  ...
  cached.forEach(day => {
    (day.countries || []).forEach(c => {
      const key = c.keys?.[0] || c.country;
      if (!key) return;
      const existing = countryMap.get(key) || { clicks: 0, impressions: 0 };
  ...
  const countries = Array.from(countryMap.entries())
    .map(([country, d]) => ({ country, clicks: d.clicks, impressions: d.impressions }))
    .sort((a, b) => b.clicks - a.clicks);

  // Striking distance queries: position 5-20, high impressions, low CTR
  const strikingDistance = queries
    .filter(q => q.avgPosition >= 5 && q.avgPosition <= 20 && q.impressions >= 10 && q.avgCtr < 0.05)
    .sort((a, b) => b.impressions - a.impressions)
    .slice(0, 15);

  // High-impression low-CTR pages
  const lowCtrPages = pages
    .filter(p => p.impressions >= 20 && p.avgCtr < 0.03)
    .sort((a, b) => b.impressions - a.impressions)
    .slice(0, 10);
