...
/**
 * Build Slack message for repeated downtime alert (Alert X of 5)
 */
export function buildRepeatedAlertEmbed(site, healthCheck, alertNumber, maxAlerts, incidentStartedAt) {
  // Escalating urgency
  let urgencyLabel
  let color
  if (alertNumber >= maxAlerts) {
    urgencyLabel = 'FINAL ALERT'
  ...
  const headerText = `${urgencyLabel} — Site Down (Alert ${alertNumber} of ${maxAlerts})`
  ...
  // Show downtime duration for alerts 2+
  const fields = [
    {
      type: 'mrkdwn',
      text: `*Environment:*\n${site.environment || 'production'}`,
  ...
  if (alertNumber >= 2 && incidentStartedAt) {
    const downtimeMs = Date.now() - new Date(incidentStartedAt).getTime()
    const downtimeMinutes = Math.floor(downtimeMs / 60000)
    const downtimeSeconds = Math.floor((downtimeMs % 60000) / 1000)
    const durationText = downtimeMinutes > 0
      ? `${downtimeMinutes}m ${downtimeSeconds}s`
      : `${downtimeSeconds}s`
    fields.push({
      type: 'mrkdwn',
      text: `*Downtime:*\n${durationText}`,
