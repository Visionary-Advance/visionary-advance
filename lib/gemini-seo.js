 ...
 */
export async function generateAIRecommendations(site, currentMonth, previousMonth, changes, { trackedKeywords = [] } = {}) {
  ...
  const dataSummary = buildDataSummary(site, currentMonth, previousMonth, changes, null, trackedKeywords);

  const trackedKeywordsGuidance = trackedKeywords.length > 0
    ? `\n\nIMPORTANT: This client has a curated list of tracked keywords (see "Tracked Keywords" section above) that they specifically care about. At least one recommendation MUST address keywords that lost positions or aren't ranking yet. Reference tracked keywords by name in the description and actions.`
    : '';
...
- Areas of concern (declining metrics)${trackedKeywordsGuidance}
 ...
 */
export async function generateSEOPlan(site, currentMonth, previousMonth, changes, { detailedAnalytics, trackedKeywords = [] } = {}) {
  const client = getClient();
  const model = client.getGenerativeModel({
    model: 'gemini-2.5-flash',
    generationConfig: {
  ...
  const dataSummary = buildDataSummary(site, currentMonth, previousMonth, changes, detailedAnalytics, trackedKeywords);
  const monthName = currentMonth?.name || new Date().toLocaleString('en-US', { month: 'long', year: 'numeric' });

  const trackedKeywordsGuidance = trackedKeywords.length > 0
    ? `\n6. PRIORITIZE the client's tracked keywords (see "Tracked Keywords" section). Tasks should target keywords that lost positions, aren't ranking yet, or are within striking distance of their target rank. Include the actual tracked keyword text in target_keywords arrays where relevant — these are the keywords the client is being measured against.`
    : '';
...
5. Reference specific pages or queries from the data where applicable${trackedKeywordsGuidance}
 ...
 */
function buildDataSummary(site, currentMonth, previousMonth, changes, detailedAnalytics, trackedKeywords = []) {
...
`;
  ...
  // Tracked keywords — the client's curated list (highest signal for what to optimize)
  if (trackedKeywords && trackedKeywords.length > 0) {
    ...
    for (const kw of trackedKeywords) {
      ...
      const target = kw.target_position;
      ...
      ranking.push(kw);
      if (kw.positionDelta !== null && kw.positionDelta !== undefined) {
        if (kw.positionDelta >= 1) winners.push(kw);
        else if (kw.positionDelta <= -1) losers.push(kw);
      }
      // Track distance-from-target separately
      kw._distanceFromTarget = target ? cur - target : null;
    }

    summary += `\n### Tracked Keywords (the client's curated targets)\n`;
    summary += `These are the ${trackedKeywords.length} keywords the client cares about most. Wins, losses, and gaps here are the primary measure of success.\n\n`;

    for (const kw of trackedKeywords) {
      const cur = kw.current?.position;
      const prev = kw.previous?.position;
      const target = kw.target_position;
      const fmt = (v) => (v === null || v === undefined ? 'not ranking' : `pos ${v.toFixed(1)}`);
      const delta = kw.positionDelta;
      const deltaStr = delta === null || delta === undefined
        ? ''
        : delta > 0
          ? ` (improved ${delta.toFixed(1)} spots)`
          : delta < 0
            ? ` (lost ${Math.abs(delta).toFixed(1)} spots)`
            : '';
      const targetStr = target ? `, target pos ${target}` : '';
      const trafficStr = kw.current
        ? `, ${kw.current.clicks || 0} clicks / ${kw.current.impressions || 0} impr`
        : '';
      summary += `- "${kw.keyword}": ${fmt(cur)} (was ${fmt(prev)})${deltaStr}${targetStr}${trafficStr}\n`;
    }

    if (notRanking.length > 0) {
      summary += `\n**Tracked keywords NOT ranking** (need new content or optimization to break in): ${notRanking.map(k => `"${k.keyword}"`).join(', ')}\n`;
    }
    if (losers.length > 0) {
      summary += `\n**Tracked keywords LOSING ground** (need urgent attention): ${losers.map(k => `"${k.keyword}" (${k.positionDelta.toFixed(1)})`).join(', ')}\n`;
    }
    if (winners.length > 0) {
      summary += `\n**Tracked keywords gaining ground** (lean into what's working): ${winners.map(k => `"${k.keyword}" (+${k.positionDelta.toFixed(1)})`).join(', ')}\n`;
    }
  }
