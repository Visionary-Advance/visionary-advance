 ...
 */
export function generateOAuthUrl(returnUrl = '/admin/crm/settings') {
  ...
  // Create signed state with timestamp and return URL
  const stateData = {
    returnUrl,
    timestamp: Date.now(),
...
export async function storeGoogleAuth(authData) {
  ...
  console.log(`Stored Google auth for: ${email}`);
  return data;
}
...
/**
 * Check Google connection status for an email
 */
export async function getGoogleConnectionStatus(email) {
  try {
    const { data, error } = await getSupabaseClient()
      .from('google_oauth')
      .select('email, status, last_used_at, created_at, error_message')
      .eq('email', email)
      .single();
