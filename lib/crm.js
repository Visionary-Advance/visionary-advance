...
import { sendN8nNotification, buildLeadNotification } from './n8n'
...
export async function createOrUpdateLead(leadData) {
  ...
  // Send n8n/Telegram notification for every submission (non-blocking)
  try {
    await sendN8nNotification(buildLeadNotification(lead, { isNew }))
  } catch (error) {
...
export async function getPipelineStats() {
  // Include won clients in stats (so won count reflects all converted clients)
  const { data, error } = await supabase
    .from('crm_leads')
    .select('stage, score, created_at, is_client')
    .not('status', 'in', '("archived","unqualified")')
    .or('is_client.is.null,is_client.eq.false,stage.eq.won')
