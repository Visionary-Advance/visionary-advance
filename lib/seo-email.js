        ...
        <table width="600" cellpadding="0" cellspacing="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);">
          ...
          <tr>
            <td style="padding: 0 40px 32px 40px;">
              <h2 style="margin: 0 0 6px 0; color: #111827; font-size: 18px; font-weight: 600;">Tracked Keywords</h2>
              <p style="margin: 0 0 16px 0; color: #6b7280; font-size: 13px;">Search ranking for the keywords we're targeting for you (lower = better).</p>
              <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse: collapse;">
                <tr>
                  <td style="padding: 10px 12px; background-color: #f9fafb; border: 1px solid #e5e7eb; font-weight: 600; color: #374151; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em;">Keyword</td>
                  <td style="padding: 10px 12px; background-color: #0d9488; border: 1px solid #0d9488; text-align: center; color: white; font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em;">${currentMonth?.name || 'Current'}</td>
                  <td style="padding: 10px 12px; background-color: #6b7280; border: 1px solid #6b7280; text-align: center; color: white; font-weight: 600; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em;">${previousMonth?.name || 'Previous'}</td>
                  <td style="padding: 10px 12px; background-color: #f9fafb; border: 1px solid #e5e7eb; text-align: center; font-weight: 600; color: #374151; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em;">Change</td>
                  <td style="padding: 10px 12px; background-color: #f9fafb; border: 1px solid #e5e7eb; text-align: center; font-weight: 600; color: #374151; font-size: 12px; text-transform: uppercase; letter-spacing: 0.04em;">Target</td>
                </tr>
                ${trackedKeywords.map((kw) => formatKeywordRow(kw)).join('')}
              </table>
            </td>
          </tr>
          ` : ''}
...
`;
  ...
  if (trackedKeywords.length > 0) {
    text += `\nTRACKED KEYWORDS\n`;
    text += `Search ranking for the keywords we're targeting (lower = better).\n\n`;
    text += `${'Keyword'.padEnd(36)}${(currentMonth?.name || 'Current').padEnd(14)}${(previousMonth?.name || 'Previous').padEnd(14)}${'Δ'.padEnd(8)}Target\n`;
    text += `─────────────────────────────────────────────────────────────────────────────\n`;
    for (const kw of trackedKeywords) {
      ...
      text += `${kw.keyword.slice(0, 35).padEnd(36)}${fmt(cur).padEnd(14)}${fmt(prev).padEnd(14)}${deltaStr.padEnd(8)}${kw.target_position ?? '—'}\n`;
    }
    text += `\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`;
  }
 ...
 */
function formatKeywordRow(kw) {
  const cur = kw.current?.position;
  const prev = kw.previous?.position;
  const delta = kw.positionDelta;
  const fmtPos = (v) => (v === null || v === undefined ? '—' : v.toFixed(1));

  let deltaCell;
  if (delta === null || delta === undefined) {
    deltaCell = '<span style="color: #6b7280;">—</span>';
  } else if (Math.abs(delta) < 0.05) {
    deltaCell = '<span style="color: #6b7280;">0</span>';
  } else {
    const color = delta > 0 ? '#22c55e' : '#ef4444';
    const arrow = delta > 0 ? '▲' : '▼';
    deltaCell = `<span style="color: ${color}; font-weight: 600;">${arrow} ${Math.abs(delta).toFixed(1)}</span>`;
  }
  ...
  return `
    <tr>
      <td style="padding: 10px 12px; border: 1px solid #e5e7eb; color: #111827; font-size: 13px;">${escapeHtml(kw.keyword)}</td>
      <td style="padding: 10px 12px; border: 1px solid #e5e7eb; text-align: center; color: #111827; font-weight: 600; font-size: 14px;">${fmtPos(cur)}</td>
      <td style="padding: 10px 12px; border: 1px solid #e5e7eb; text-align: center; color: #6b7280; font-size: 13px;">${fmtPos(prev)}</td>
      <td style="padding: 10px 12px; border: 1px solid #e5e7eb; text-align: center; font-size: 13px;">${deltaCell}</td>
      <td style="padding: 10px 12px; border: 1px solid #e5e7eb; text-align: center; color: #6b7280; font-size: 13px;">${target}</td>
    </tr>
  `;
}
