...
//     // Priority: 1. Env var, 2. Detection, 3. Hardcoded
...
//       } else {
//         // HARDCODE production URL to ensure https://
...
export async function GET(request) {
  try {
    ...
    console.log('✅ Callback GET route hit');
    console.log('Code:', code);
    console.log('Error:', error);
    
    if (error) {
      return NextResponse.redirect(
        new URL(`/square/error?error=${error}`, request.url)
      );
    }
    
    if (!code) {
      return NextResponse.redirect(
        new URL('/square/error?error=no_code', request.url)
      );
    }
    
    // For now, just redirect to success to confirm routing works
    return NextResponse.redirect(
      new URL(`/square/success?merchant_id=test&client_id=test&location_count=0`, request.url)
    );
    
  } catch (err) {
    console.error('Callback error:', err);
    return NextResponse.redirect(
      new URL('/square/error?error=server_error', request.url)
    );
  }
}
